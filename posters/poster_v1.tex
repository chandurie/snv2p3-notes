...
\newcommand{\metriccard}[2]{%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.8em,center,rounded=true,shadow=false,fg=royal,bg=white!96!mint]{block body}%
    {\Large\bfseries\color{accent}#1}\\[-0.25em]
    {\footnotesize #2}%
  \end{beamercolorbox}%
}
\newcommand{\callout}[2]{%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.6em,rounded=true,shadow=false,fg=royal,bg=white!95!accent]{block body}%
...
\title{\textbf{Refining the Sunspot Number Series to SN~V3}}
...
%      \callout{SN heritage}{1607--1980 primary sources}\\[0.4cm]
%      \callout{Release goal}{SN V3 with open uncertainties}
    \end{minipage}
  \end{beamercolorbox}%
  \vspace{0.8cm}
}
...
\setbeamertemplate{footline}{%
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=1.3cm,dp=0ex,leftskip=2.2cm,rightskip=2.2cm]{titlewrap}
    \vspace{0.2cm}
    \small
    \textbf{Contact}\,\,: \href{mailto:sidc-support@oma.be}{sidc-support@oma.be}\hfill
...
\begin{tightblock}{Why Reconstruction?}
  The SN inherits observer-dependent scale changes (for example the 1947 "Waldmeier jump") that cannot be removed by tweaking coefficients alone. FARSuN pioneers a full reconstruction that replays the entire observing chain.
\begin{itemize}
  ...
  \item Direct access to the raw sources lets us audit outliers, model observer response curves, and issue reproducible releases such as SN~V3.
\end{itemize}
\end{tightblock}
...
\begin{tightblock}{FARSuN Historical Sunspot Database} % — Status \& Outlook}
  ...
  \begin{itemize}
    ...
    \item Use this harmonized database as the foundation for SN V3 calibration and long-term solar activity studies.
  \end{itemize}
...
\begin{tightblock}{Key Datasets}
  \datasetbox{Mittheilungen Dataset}{codegreen}
% \vspace{0.3em}
...
%   \item Inclusion of Mittheilungen as the core calibration reference for SN Version 3.
...
%   \item Deliver public access to the curated Z\"urich dataset to support reconstruction of SN~V3.
% \end{itemize}
\begin{figure}[t]
  \centering
  \includegraphics[width=0.65\linewidth]{./figures/zurich_tables.png}
...
\end{figure}
  ...
  \datasetbox{C.~H. Adams Data}{codepurple} % — Present Status \& Future Work
% \vspace{0.3em}
...
\datasetbox{Gruithuisen Data}{codeorange}% — Present Status \& Future
% \vspace{0.3em}
The Gruithuisen manuscripts (1817--1849) capture daily sunspot notes, drawings, and weather context in German Kurrent script, supplying early-19th-century coverage vital for SN~V3 refinement.\\
...
%   \item Publish FAIR, VO-ready daily tables and image catalogues via ROB/SILSO to support SN~V3 calibration and uncertainty modelling.
% \end{itemize}
\begin{figure}[t]
  \centering
...
\begin{column}{0.27\paperwidth}
\begin{tightblock}{Key Datasets}
\datasetbox{Augustin Stark Data}{codered}
% \vspace{0.3em}
The Stark dataset (1813–1835), comprising printed German descriptions of sunspot groups and chains, has been partially retrieved. Preliminary OCR and extraction tests show good potential, and ongoing work focuses on refining consistency, QC flags, and metadata integration into the FARSuN database.
% \vspace{0.9em}
\datasetbox{Summary}{codegray}
...
Gruithuisen & 1817--1849 & 100\% digitized; Extraction using transkribus like models + dual QC for group/spot counts underway. \\
C. H. Adams & 1819--1823 & 338 sketches extracted (1\,056 entries); spotless days cross-checked against Wolf series. \\
Augustin Stark & 1813--1835 & Trial extraction are underway. \\
...
\begin{itemize}
  ...
  \item Zürich pipeline combines Transkribus extraction with manual review to reach the 2026 target.
...
\end{itemize}
\end{tightblock}
\begin{tightblock}{From Raw to SN V3}
\begin{enumerate}
  ...
  \item Benchmark against SN~V2 and satellite-era proxies to validate drift corrections.
\end{enumerate}
...
\begin{itemize}
  \item Decode \emph{Kurrent} manuscripts while preserving marginal notes.
  \item Harmonise drawings, ledgers, and weather annotations.
  \item Quantify observer bias and spotless-day reporting habits.
  \item Maintain provenance from scanned folio to released product.
\end{itemize}
\end{tightblock}
...
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
  \callout{\color{codeblue}Forecasting}{Bias-free SN~V3 improves solar cycle prediction skill and operational space weather baselines.}
\end{minipage}\\[0.4em]
\begin{minipage}[t]{0.48\linewidth}
...
%   \item Publish SN~V3 release notes covering uncertainties and API endpoints.
...
\centering
\includegraphics[width=0.65\linewidth]{./docs/figures/qrcode_farsun-poster.png}\\[0.3em]
\small For more information on FARSuN project, Different datasets, softcopy of the poster, scan this QR code.
\end{tightblock}
\end{column}
