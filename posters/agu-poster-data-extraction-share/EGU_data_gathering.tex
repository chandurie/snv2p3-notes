...
\definecolor{EGUroyal}{HTML}{0B132B}        % deep midnight blue
\definecolor{EGUaccent}{HTML}{F15152}       % vibrant coral
\definecolor{EGUsoftbg}{HTML}{F7FAFC}       % airy background
\definecolor{EGUmint}{HTML}{06AED5}         % bright aqua
\definecolor{EGUhighlight}{HTML}{FFC857}    % warm highlight
\definecolor{EGUviolet}{HTML}{5B2E90}       % punchy violet
\definecolor{EGUdanger}{HTML}{F46036}       % bold alert orange
\definecolor{EGUcolumnblue}{HTML}{E6F6FA}   % column wash
\definecolor{EGUdatasetgreen}{HTML}{2CB978} % dataset tab green
\definecolor{EGUcodegreen}{HTML}{2CB978}
\definecolor{EGUcodeblue}{HTML}{0F8BCE}
\definecolor{EGUcodepurple}{HTML}{6C4AB6}
\definecolor{EGUcodeorange}{HTML}{F9A620}
\definecolor{EGUcodered}{HTML}{F15152}
\definecolor{EGUcodegray}{HTML}{6B7B8A}
\definecolor{EGUcodecomment}{HTML}{2F8F83}
\definecolor{EGUcodestring}{HTML}{E85D75}
% Colors (EGU visual identity inspired)
\definecolor{royal}{HTML}{0072BC}        % EGU main blue
\definecolor{accent}{HTML}{FFDE00}       % EGU yellow
\definecolor{softbg}{HTML}{F7FAFC}       % airy background
\definecolor{mint}{HTML}{A8A9AD}         % EGU neutral gray
\definecolor{highlight}{HTML}{FFDE00}    % EGU yellow highlight
\definecolor{violet}{HTML}{0072BC}       % harmonised with primary blue
\definecolor{danger}{HTML}{0072BC}       % keep emphasis within brand palette
\definecolor{columnblue}{HTML}{EAF4FB}   % pale blue column wash
\definecolor{datasetgreen}{HTML}{0072BC} % align tabs with EGU blue
\definecolor{codegreen}{HTML}{0072BC}
\definecolor{codeblue}{HTML}{0072BC}
\definecolor{codepurple}{HTML}{0072BC}
\definecolor{codeorange}{HTML}{FFDE00}
\definecolor{codered}{HTML}{0072BC}
\definecolor{codegray}{HTML}{6B7B8A}
\definecolor{codecomment}{HTML}{6B7B8A}
\definecolor{codestring}{HTML}{0072BC}
\definecolor{egugray}{HTML}{A8A9AD}
...
\author{\texorpdfstring{\large Laure Lef\`evre\textsuperscript{1}, \textcolor{highlight}{\textbf{Kalugodu Chandrashekhar}}\textsuperscript{1}, Benjamin Mampaey\textsuperscript{1}, Senthamizh Pavai Valliappan\textsuperscript{1}, Hisashi Hayakawa\textsuperscript{2}, Christian Ritter\textsuperscript{3}}{\normalsize Chandrashekhar (1), Lefevre (1), Mampaey (1), Pavai Valliappan (1), Hayakawa (2), Ritter (3)}}
...
\newcommand{\egulogo}{logos/egu_claim_yellow.png}
...
%        {\bfseries\small EGU26-17694 - May 6 2025, 14:00--18:00 - Poster Hall X4}\\[4pt]
%        {\small Session: ST4.1 - Open Session on Space Weather and Space Climate}
        {\bfseries\tiny See talk EGU26-19117 -Sunspot Number V2.0 Through Solar Cycle 25: A Long-term Multi-Proxy Stability Analysis.}\\[4pt]
        {\tiny May 4th 2026 14:35 - Session: ST4.1 - Open Session on Space Weather and Space Climate}
        % {\bfseries\small SH23E-2613*}\\[4pt]
        % {\small ``The Future of the International Sunspot Number''}\\[-0.05cm]
        % {\small 16 Dec 2025 $\bullet$  14:15--17:45 CST}
...
\setbeamertemplate{footline}{%
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=1.9cm,dp=0ex,leftskip=2.2cm,rightskip=2.2cm]{titlewrap}
    \vspace{0.18cm}
    \LARGE
    {\color{highlight}\textbf{Contact}:} \href{mailto:silso.info@oma.be}{\textbf{silso.info@oma.be}}\hfill
%    \includegraphics[height=2cm]{\egulogo}\hfill  EGU26-17694 \hfill 
    \hfill  EGU26-17694 \hfill 
...
\newenvironment{tightblock}[1]{%
  \begin{tcolorbox}[
    ...
    title=#1,
    fonttitle=\bfseries\large\color{accent!20!white},
    ...
    boxed title style={
      ...
      left=8pt,
      right=8pt,
      fontupper=\bfseries\large\color{accent!20!white}
    }
  ]
}{\end{tcolorbox}}
...
To enable this reconstruction, the historical dataset has expanded dramatically in recent years. WDC--SILSO digitized the Mittheilungen (1610--1944) between 2017 and 2019, converting Wolf's original Z\"urich journals into machine-readable datasets with modern metadata and provenance. As part of FARSUN, the Z\"urich observation tables for 1945--1979 were digitized from 2023 to 2025, completing the recovery of the full Z\"urich legacy. In parallel, major community efforts (e.g., Hayakawa, Arlt, Vaquero, Carrasco, Vokhmyanin & Zolotova) have located, transcribed, and published numerous additional early telescopic sunspot records since 2010.
...
% \end{minipage}\hfill
% \begin{minipage}[t]{0.48\linewidth}
%   \metriccard{2\,000+}{Z\"urich observation tables tracked}
% \end{minipage}\\[0.6em]
% \begin{minipage}[t]{0.48\linewidth}
...
\begin{tightblock}{Methodology}
\begin{enumerate}
  \item \key{Collect Sources}: gather historical observing records from many archives (e.g. Wolf's \emph{Mittheilungen}, Z\"urich tables, manuscripts, drawings) and record where each piece of data comes from.
  \item \key{Digitize Data}: convert handwritten and printed records into digital form using handwriting recognition tools and careful manual entry when needed.
  \item \key{Check \& Align}: verify the data, resolve different names for the same observers, align dates and formats, and account for how different observers counted sunspots.
  \item \key{Share Results}: provide the data in easy-to-use formats, including online access tools (APIs), and include uncertainty estimates so users can assess how reliable the data are.
\end{enumerate}
...
\begin{tightblock}{FARSuN Historical Sunspot Database} % --- Status \& Outlook}
  ...
  \begin{itemize}
    \item Legacy sources (Mitteilungen, Wolf Institute, etc.) have been integrated into the current schema for cross-validation and quality control.
  ...
  \end{itemize}
Next steps include finalizing the data schema and completing links to legacy data sets, expanding automated quality-control (QC) and improving uncertainty modeling, and increasing digitization and data extraction (Adams, Gruithuisen, Stark, etc.) using handwriting recognition (HTR) and citizen-science contributions. The full data set will be made available through the EPN-TAP service, following FAIR (Findable, Accessible, Interoperable, Reusable) principles. This harmonized database will then support the $S_{N}$ V3.0 calibration and long-term solar activity studies.
%\begin{itemize}
%  \item Finalize the canonical data schema and complete the remaining links to legacy data sets.
%  \item Expand automated quality-control (QC) dashboards and improve uncertainty modeling.
%  \item Increase digitization and data extraction (Adams, Gruithuisen, Stark, etc.) using handwriting recognition (HTR) and citizen-science contributions.
%  \item Provide access to the full data set through the EPN-TAP service, ensuring FAIR (Findable, Accessible, Interoperable, Reusable) data standards.
%  \item Use this harmonized database as the basis for the $S_{N}$ V3.0 calibration and for studies of long-term solar activity.
%\end{itemize}
\end{tightblock}

\begin{tightblock}{Digitize and Ingest data}
\begin{itemize}
  \item Transcription: combine handwriting recognition (OCR) with manual entry for complex tables.
  \item OCR on existing online Mittheilungen journals to extract metadata automatically. 
  \item Combine extracted data into homogeneous format to ingest into the database. 
%  \item Automation: use reproducible Python workflows with clear steps for data entry and review, including citizen-science contributions.
\end{itemize}
\end{tightblock}

\begin{tightblock}{Quality Control}
\begin{itemize}
  \item Verification across same time observers: e.g. counts from Adams (1819-1823) and Gruithuisen (1817-1849) are checked independently before deriving indices.
  \item Cross-checks: compare FARSuN transcriptions with Wolf's \emph{Mittheilungen} and modern SILSO indices to detect inconsistencies over time.
  \item Metadata consistency: ensure each observer has a unique ID, and preserve information on instruments, location, and observing conditions.
  \item Reproducibility: track all data processing steps with Git and notebooks so results can be checked and reused.
\end{itemize}
\end{tightblock}
\end{column}
...
\vspace{ 0.4em}
  \datasetbox{Mittheilungen Dataset (1610--1944)}{EGUcodegreen}
\vspace{ 0.4em}
...
% % \textbf{Current focus}
\begin{itemize}
  \item Refining data consistency and observer calibration (e.g. Wolf--Wolfer overlap).
...
\end{figure}
\vspace{-01.4em}
\datasetbox{Z\"urich Tables (1945--1979)}{EGUcodeblue}
...
\textit{Status}: All 1500+ yearly tables are digitized and the remaining observer metadata are being encoded.
\vspace{0.3em}
...
\begin{itemize}
  \item Image sets and text snippets fully extracted and collected with preliminary CSV/XLSX extraction completed.
  \item Two-pass quality control (survey + counting) with verified sunspot counts in progress.
\end{itemize}
...
\vspace{0.5em}
  \datasetbox{C.~H. Adams Data (1819--1823)}{EGUcodepurple} % --- Present Status \& Future Work
\vspace{0.3em}
...
All spotless-day counts are cross-checked against Wolf's compilations, anchoring early-1800s calibration experiments.
...
\begin{itemize}
  ...
  \item Orientation, and standardisation of drawings to enable positions and areas extraction.
\end{itemize}
% \begin{figure}[t]
%   \centering
% adams_example.png is not present in this repository snapshot.
% \end{figure}
% \\
...
{\small
\begin{tightblock}{Key Datasets}
\datasetbox{Augustin Stark Data (1813--1835)}{EGUcodered}
{
...
\datasetbox{Community Data Recovery Efforts Since 2010}{codecomment}
{
...
\end{tightblock}
%\begin{tightblock}{Progress}
%\begin{itemize}
%  \item Z\"urich pipeline combines automatic OCR extraction with manual review to have all data in the DB before the end of 2026.
...
%  \item Stark data trials to assess what information can be extracted
%  \item Adams positions and sizes extraction in progress
%\end{itemize}
%\end{tightblock}
...
\begin{tightblock}{From Raw Sunspot Data to $S_{N}$ V3.0}
\begin{enumerate}
  \item Focus on digitizing the most difficult periods, where it meaningfully improves the final series.
  \item Curate daily counts with provenance-rich metadata (observer, instrument, location, context).
  \item Model how each observer counted sunspots, and use this to adjust their data so observations from different centuries can be compared on the same scale.
  \item Aggregate to monthly, yearly, and hemispheric indices when positions are available with propagated uncertainties.
  \item Benchmark against $S_{N}$~V2.0 and long-term proxies (e.g. sunspot areas, magnetic flux, F10.7) to validate drift corrections.
\end{enumerate}
...
\begin{minipage}[t]{0.48\linewidth}
  \callout{\color{EGUcodestring}Citizen science}{Interactive portal to mobilise volunteers to extract complex data from images and manuscripts.}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
  \callout{\color{EGUcodeblue}Forecasting}{Bias-free $S_{N}$~V3.0 will improve solar cycle prediction skills and operational space weather baselines.}
\end{minipage}
\par\vspace{0.25em}
\begin{minipage}[t]{0.48\linewidth}
  \callout{\color{EGUcodeorange}Climate links}{Validated historical forcing refines long-term climate models and Earth radiation budget studies.}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
  \callout{\color{EGUcodegreen}Open science}{FAIR-compliant releases make the data easy to find and use, with online access tools and clear uncertainty estimates.}
\end{minipage}
\end{tightblock}
